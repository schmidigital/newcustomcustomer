...
require_once "Mage/Customer/controllers/AccountController.php";
class Newcustom_Customer_Customer_AccountController extends Mage_Customer_AccountController{
 ...
 /**
        ...
        if ($this->getRequest()->isPost()) {
            ...
            if (!empty($login['username']) && !empty($login['password'])) {
                try {
                    ...
                    // Added by Egon Schmid
                    $customer = $session->getCustomer();
                    $groupId = $customer->getGroupId();
                    $origId =  $customer->getOrigGroupId();
                    if(!$origId) {
                        $customer->setOrigGroupId($origId = $groupId);
                        $customer->save();
                    }
                    if($groupId != $origId) {
                        $customer->setGroupId($groupId = $origId);
                    }
                    $session->setCurrentGroupId($gid);
    ...
    protected function _loginPostRedirect()
    {
        ...
        if (!$session->getBeforeAuthUrl() || $session->getBeforeAuthUrl() == Mage::getBaseUrl()) {
            ...
            // $session->setBeforeAuthUrl($this->_getHelper('customer')->getAccountUrl()); 
                ...
                $storecode = Mage::getModel('core/store')->load($storeid, 'store_id')->getCode(); 
                $customurl = Mage::getBaseUrl().'?___store='.$storecode.'&___from_store='.$currentstoreCode; 
                $session->setBeforeAuthUrl($customurl); 
            ...
            if ($session->isLoggedIn()) {
                ...
                $storeid =  $customerData->getStoreId(); 
                $storecode = Mage::getModel('core/store')->load($storeid, 'store_id')->getCode(); 
                $customurl = $session->getAfterAuthUrl(true).'?___store='.$storecode.'&___from_store='.$currentstoreCode; 
                $session->setBeforeAuthUrl($customurl);
       ...
       // if($session->getBeforeAuthUrl(true) == Mage::getBaseUrl().'/customer/account/'){echo "test";}
    ...
    public function logoutAction()
    {
        ...
        $session = $this->_getSession();
        $customer = $session->getCustomer();
        $customer->setGroupId($customer->getOrigGroupId());
        $customer->save();
